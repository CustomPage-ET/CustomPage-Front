...
export default function GestionarContenidoPage() {
  ...
  // Listas de colores recientes dinámicas para los círculos
  ...
  // 2. ESTADOS DE ESTILO VISUAL Y CONTACTO
  ...
  // Cargar configuraciones guardadas al iniciar la vista
  useEffect(() => {
    ...
    <div className="w-full text-slate-800">
      ...
      <main className="w-full">
        <div className="flex items-center justify-between mb-6">
          ...
          {successMessage && (
            <div className="p-3 bg-emerald-50 border border-emerald-200 text-emerald-800 text-xs font-bold rounded-xl shadow-sm">
              ✅ {successMessage}
            </div>
          )}
        </div>
        ...
        <form onSubmit={handleSaveChanges} className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            ...
            <div className="flex flex-col sm:flex-row gap-6 items-start">
              ...
              <div className="flex-1 space-y-5 w-full">
                <div>
                  ...
                  <input type="text" value={siteName} onChange={(e) => { setSiteName(e.target.value); localStorage.setItem('web_site_name', e.target.value); }} className="w-full px-4 py-2.5 rounded-xl bg-white border border-slate-200 text-xs font-bold text-slate-700 focus:outline-none" />
                </div>

                {/* FILA 1: COLOR DE BOTONES */}
                <div className="flex flex-col gap-1.5">
                  <label className="block text-xs font-bold text-slate-500">Color de Botones y Acciones:</label>
                  <div className="flex items-center gap-2.5">
                    <div className="flex items-center gap-1.5">
                      {presetButtons.map((color) => (
                        <button
                          key={color}
                          type="button"
                          onClick={() => handleColorSelection(color, 'web_button_color', setButtonColor, presetButtons, setPresetButtons, 'history_buttons')}
                          className={`w-7 h-7 rounded-full border transition-all cursor-pointer ${buttonColor.toLowerCase() === color.toLowerCase() ? 'border-slate-800 scale-110 shadow-md ring-1 ring-slate-400' : 'border-slate-200 hover:scale-105'}`}
                    ...
                    <label className="w-7 h-7 rounded-full border border-slate-300 flex items-center justify-center bg-white hover:bg-slate-100 active:scale-95 transition-all cursor-pointer text-slate-500 text-sm font-bold shadow-sm">
                      ...
                      <input
                        type="color"
                        value={buttonColor}
                        onChange={(e) => handleColorSelection(e.target.value, 'web_button_color', setButtonColor, presetButtons, setPresetButtons, 'history_buttons')}
                        className="sr-only"
                      />
                    </label>
                ...
                {/* FILA 2: COLOR DE TEXTOS PRINCIPALES */}
                <div className="flex flex-col gap-1.5">
                  <label className="block text-xs font-bold text-slate-500">Color de Títulos y Textos Principales:</label>
                  <div className="flex items-center gap-2.5">
                    <div className="flex items-center gap-1.5">
                      {presetTexts.map((color) => (
                        <button
                          key={color}
                          type="button"
                          onClick={() => handleColorSelection(color, 'web_main_text_color', setMainTextColor, presetTexts, setPresetTexts, 'history_texts')}
                          className={`w-7 h-7 rounded-full border transition-all cursor-pointer ${mainTextColor.toLowerCase() === color.toLowerCase() ? 'border-slate-800 scale-110 shadow-md ring-1 ring-slate-400' : 'border-slate-200 hover:scale-105'}`}
                          style={{ backgroundColor: color }}
                        />
                      ))}
                    </div>
                    <label className="w-7 h-7 rounded-full border border-slate-300 flex items-center justify-center bg-white hover:bg-slate-100 active:scale-95 transition-all cursor-pointer text-slate-500 text-sm font-bold shadow-sm">
                      ...
                      <input
                        type="color"
                        value={mainTextColor}
                        onChange={(e) => handleColorSelection(e.target.value, 'web_main_text_color', setMainTextColor, presetTexts, setPresetTexts, 'history_texts')}
                        className="sr-only"
                      />
                    </label>
                    <span className="text-[10px] font-mono font-extrabold text-rose-600 bg-rose-50 px-2 py-1 rounded-md border border-rose-100 shadow-sm">
                      {mainTextColor.toUpperCase()}
                    </span>
                  </div>
                </div>

                {/* FILA 3: COLOR DE CASILLAS Y FONDOS */}
                <div className="flex flex-col gap-1.5">
                  <label className="block text-xs font-bold text-slate-500">Color de Casillas, Cuadrados y Fondos:</label>
                  <div className="flex items-center gap-2.5">
                    <div className="flex items-center gap-1.5">
                      {presetBackgrounds.map((color) => (
                        <button
                          key={color}
                          type="button"
                          onClick={() => handleColorSelection(color, 'web_box_bg_color', setBoxBgColor, presetBackgrounds, setPresetBackgrounds, 'history_backgrounds')}
                          className={`w-7 h-7 rounded-full border transition-all cursor-pointer ${boxBgColor.toLowerCase() === color.toLowerCase() ? 'border-slate-800 scale-110 shadow-md ring-1 ring-slate-400' : 'border-slate-200 hover:scale-105'}`}
                          style={{ backgroundColor: color }}
                        />
                      ))}
                    </div>
                    <label className="w-7 h-7 rounded-full border border-slate-300 flex items-center justify-center bg-white hover:bg-slate-100 active:scale-95 transition-all cursor-pointer text-slate-500 text-sm font-bold shadow-sm">
                      ...
                      <input
                        type="color"
                        value={boxBgColor}
                        onChange={(e) => handleColorSelection(e.target.value, 'web_box_bg_color', setBoxBgColor, presetBackgrounds, setPresetBackgrounds, 'history_backgrounds')}
                        className="sr-only"
                      />
                    </label>
                    <span className="text-[10px] font-mono font-extrabold text-rose-600 bg-rose-50 px-2 py-1 rounded-md border border-rose-100 shadow-sm">
                      {boxBgColor.toUpperCase()}
                    </span>
                  </div>
                </div>
              </div>
            ...
            <div className="space-y-3 pt-4 border-t border-slate-200">
              ...
              <div className="flex flex-col sm:flex-row gap-4 items-start">
                ...
                <div className="flex-1 w-full">
                  <textarea rows={4} placeholder="Escribe el texto corporativo de presentación..." value={aboutText} onChange={(e) => { setAboutText(e.target.value); localStorage.setItem('web_about_text', e.target.value); }} className="w-full p-4 rounded-2xl bg-white border border-slate-200 text-xs font-medium text-slate-700 focus:outline-none resize-none" />
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="bg-slate-50/50 p-6 rounded-[28px] border border-slate-200/60 shadow-sm flex flex-col justify-between gap-6">
            ...
            <div className="pt-4 border-t border-slate-200 flex justify-end">
              <button type="submit" className="px-8 py-3.5 bg-emerald-500 hover:bg-emerald-600 text-white font-extrabold text-sm rounded-full shadow-sm transition-transform active:scale-95">
                Guardar todo
              </button>
            </div>
          </div>
